...
static const char* d_flipflop_async="D-flipflop";
static const char* d_flipflop_sync="D-flipflop-sync";
...
static const char* PullLow="Pull Low";
static const char* PullHigh="Pull High";
...
Chip* BaseChipFactory::build(const char* aChipId)
{
	...
	if(strcmp(d_flipflop_async,aChipId)==0) return new DFlipFlop(ASYNCCLR);

	if(strcmp(d_flipflop_sync,aChipId)==0) return new DFlipFlop(SYNCCLR);
	...
	if(strcmp(PullHigh,aChipId)==0) return new PullHighChip();
	if(strcmp(PullLow,aChipId)==0) return new PullLowChip();
...
void BaseChipFactory::getSupportedChips(vector<char*> aChipList[5])
{
	...
	aChipList[0].push_back(mystrdup(d_flipflop_async));
	aChipList[0].push_back(mystrdup(d_flipflop_sync));
	...
	aChipList[4].push_back(NULL);
	aChipList[4].push_back(mystrdup(PullLow));
	aChipList[4].push_back(mystrdup(PullHigh));
	aChipList[4].push_back(NULL);
