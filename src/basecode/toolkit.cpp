...
//    const SDL_RendererInfo* info2=NULL;
    int bpp = 0;
    int flags = 0;

//    info=SDL_GetVideoInfo();
    SDL_GetRenderDriverInfo(gRenderDriverIndex,info);
    ...
    SDL_GetDisplayMode(gRenderDriverIndex,0,&mode);
...
//        gScreenHeight=info->current_h;
 ...
 //       bpp=mode.format;
        ...
        //flags=SDL_WINDOW_OPENGL|SDL_FULLSCREEN;
...
//        bpp = info->vfmt->BitsPerPixel;
  ...
  //  if (SDL_SetVideoMode(gScreenWidth, gScreenHeight, bpp, flags) == 0) 
    if(SDL_CreateWindow("Something Something Darkside",SDL_WINDOWPOS_UNDEFINED,SDL_WINDOWPOS_UNDEFINED,gScreenWidth,gScreenHeight,flags)==0)
    {
...
static void do_loadtexture(texpair &Tex)
{
    ...
    if(!Tex.RawBitMap)
    {
        Tex.RawBitMap=stbi_load(Tex.mFilename,&Tex.mxSize,&Tex.mySize,&n,4);

        if(Tex.RawBitMap==NULL)
            return;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,Tex.mxSize,Tex.mySize, 0, GL_RGBA, GL_UNSIGNED_BYTE, (GLvoid*)Tex.RawBitMap);
#if do_mip_stuff
    if(!Tex.mip)
    {
...
GLuint FindTextBitmap(const char* aFilename,unsigned char **RawBitMap)
{
    ...
    for(i=0; i<gTextureStoreSize; i++)
    {
        ...
        {
            *RawBitMap=gTextureStore[i].RawBitMap;
...
GLuint EmptyTextureStore()
{
    ...
    for(i=0; i<gTextureStoreSize; i++)
    {
        ...
        if(gTextureStore[i].RawBitMap)
        {
            delete(gTextureStore[i].RawBitMap);
        }
...
void reload_textures()
{
 ...
 //           do_loadtexture(gTextureStore[i].mFilename,gTextureStore[i].mClamp);
        ...
        {
//            load_blank_texture(gTextureStore[i].mFilename,gTextureStore[i].mClamp,gTextureStore[i].mxSize,gTextureStore[i].mySize,gTextureStore[i].RawBitMap);
...
static void load_blank_texture(texpair &Tex)
{
    ...
    glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,Tex.mxSize,Tex.mySize,0,GL_RGBA,GL_UNSIGNED_BYTE,Tex.RawBitMap);
