...
SRNegChip::SRNegChip()
{
    ...
    mInputPinB.set(0, 1.25, this, "Reset");
    ...
    mTexture = load_file_texture("data/sr_neg.png");
    mOldState = 0;
...
void SRNegChip::render(int aChipId)
{
	if (gBlackBackground)
...
void SRNegChip::update(float aTick) 
{
    if (mInputPinA.mNet == NULL ||
        mInputPinB.mNet == NULL ||
//        (mInputPinA.mNet->mState == NETSTATE_LOW &&
//        mInputPinB.mNet->mState == NETSTATE_LOW) ||
