...
NORChip::NORChip(int aUS)
{
    ...
    if (aUS)
        mTexture = load_file_texture("data/nor_us.png");
    else
        mTexture = load_file_texture("data/nor_fi.png");

	mInputPinA.mReadOnly = 1;
	mInputPinB.mReadOnly = 1;
}

void NORChip::render(int aChipId)
{
	if (gBlackBackground)
...
void NORChip::update(float aTick) 
{
