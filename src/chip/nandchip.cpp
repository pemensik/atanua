...
NANDChip::NANDChip(int aUS)
{
    ...
    if (aUS)
        mTexture = load_file_texture("data/nand_us.png");
    else
        mTexture = load_file_texture("data/nand_fi.png");

	mInputPinA.mReadOnly = 1;
	mInputPinB.mReadOnly = 1;
}

void NANDChip::render(int aChipId)
{
	if (gBlackBackground)
...
void NANDChip::update(float aTick) 
{
