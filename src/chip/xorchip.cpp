...
XORChip::XORChip(int aUS)
{
    ...
    if (aUS)
        mTexture = load_file_texture("data/xor_us.png");
    else
        mTexture = load_file_texture("data/xor_fi.png");

	mInputPinA.mReadOnly = 1;
	mInputPinB.mReadOnly = 1;
}

void XORChip::render(int aChipId)
{
	if (gBlackBackground)
...
void XORChip::update(float aTick) 
{
