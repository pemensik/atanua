...
ORChip::ORChip(int aUS)
{
    ...
    if (aUS)
        mTexture = load_file_texture("data/or_us.png");
    else
        mTexture = load_file_texture("data/or_fi.png");

	mInputPinA.mReadOnly = 1;
	mInputPinB.mReadOnly = 1;
}

void ORChip::render(int aChipId)
{
	if (gBlackBackground)
...
void ORChip::update(float aTick) 
{
