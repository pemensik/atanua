...
SERChip::SERChip()
{
    ...
    mTexture = load_file_texture("data/ser.png");
...
void SERChip::render(int aChipId)
{
	if (gBlackBackground)
	    drawtexturedrect(mTexture,mX,mY,mW,mH+1,0xffffffff);
	else
		drawtexturedrect(mTexture,mX,mY,mW,mH+1,0xff000000);
}

void SERChip::update(float aTick) 
{
    if (mInputPinA.mNet == NULL ||
        mInputPinB.mNet == NULL ||
        mInputPinC.mNet == NULL ||
//        (mInputPinA.mNet->mState == NETSTATE_HIGH &&
//        mInputPinB.mNet->mState == NETSTATE_HIGH &&
//        mInputPinC.mNet->mState == NETSTATE_HIGH) ||
    ...
    if (mInputPinB.mNet->mState == NETSTATE_HIGH)
    {
        ...
        if (mInputPinC.mNet->mState == NETSTATE_HIGH)
            mOldState = 0;
    }
