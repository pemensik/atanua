...
NOTChip::NOTChip(int aUS)
{
    ...
    if (aUS)
        mTexture = load_file_texture("data/not_us.png");
    else
        mTexture = load_file_texture("data/not_fi.png");

	mInputPin.mReadOnly = 1;
}

void NOTChip::render(int aChipId)
{
	if (gBlackBackground)
...
void NOTChip::update(float aTick) 
{
    if (mInputPin.mNet == NULL ||
//        mInputPin.mNet->mState == NETSTATE_NC ||
    ...
    if (mInputPin.mNet->mState == NETSTATE_HIGH || mInputPin.mNet->mState == NETSTATE_NC)
