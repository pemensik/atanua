...
JKFlipFlop::JKFlipFlop()
{
    ...
    mTexture = load_file_texture("data/jk_flipflop.png");
    mState = 0;
    mClock = 0;

	mInputPinJ.mReadOnly = 1;
	mInputPinK.mReadOnly = 1;
	mInputPinR.mReadOnly = 1;
	mInputPinS.mReadOnly = 1;
}

void JKFlipFlop::render(int aChipId)
{
	if (gBlackBackground)
...
void JKFlipFlop::update(float aTick) 
{
    // Let's make SR pins optional, but still validated
    if (mInputPinJ.mNet == NULL ||
    ...
    if (mClockPin.mNet->mState == NETSTATE_HIGH)
    {
        ...
        {
            mClock = 1;
            if (mInputPinJ.mNet->mState == NETSTATE_HIGH && 
                mInputPinK.mNet->mState == NETSTATE_HIGH)
            {
                mState = !mState;
            }
            else
            {
                if (mInputPinJ.mNet->mState == NETSTATE_HIGH)
                    mState = 1;
                if (mInputPinK.mNet->mState == NETSTATE_HIGH)
                    mState = 0;
            }
        }
    }
